...
import { ClientIp, LoginAttemptIdentity } from './types/login-attempt.types';
...
export class AuthService {
   ...
   */
  async login(dto: LoginDto, ip: ClientIp): Promise<LoginResult> {
    const identity: LoginAttemptIdentity = {
      tenantId: toTenantId(this.tenantContext.requireTenantId()),
      email: toUserEmail(dto.email),
      ip,
    };
    await this.loginThrottle.assertNotLocked(identity);

    const user = await this.usersService.findByEmailWithSecret(identity.email);
    // Uniform error whether the user is missing, inactive, or the password is
    // wrong — don't leak which accounts exist. Unknown accounts are counted too,
    // so a locked response cannot be used to probe which emails exist.
    if (!user || !user.isActive) {
      await this.loginThrottle.recordFailure(identity);
    ...
    await this.loginThrottle.reset(identity);
