...
- **Users module** (per HCRM-32, `backend/src/users`): `GET /users` (paginated), `GET /users/:id`, `POST /users`, `PATCH /users/:id` — all behind `@RequirePermissions(users:manage)`, all tenant-scoped via RLS. Listing takes `page` (default 1) and `limit` (default 20, **capped at 100**) and returns a `PaginatedResult`, so `page/limit/total/hasMore` land in the envelope's `meta`; the page and its `count` are read in the **same** transaction, and ordering carries `id` as a tiebreaker after `createdAt desc` so rows cannot shuffle between pages. **There is no delete** — deactivation is `PATCH { isActive: false }`, and an inactive user is refused at login. `PATCH` cannot change `email` or the password (neither is in `UpdateUserDto` or `UserUpdateRecord`). **A `role` change without an explicit `permissions` list re-seeds that role's defaults** — otherwise demoting a manager would leave `users:manage` behind, and `permissions` is what `PermissionsGuard` actually reads. Updates go through `updateMany` rather than `update`: a bare `update` matches on the primary key alone, which RLS turns into a Prisma "record not found" crash for another tenant's id, whereas `updateMany` reports zero rows and the service answers a clean 404.
